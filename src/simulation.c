...
time in ms 
philos id
current action(eating, sleeping, died)
*/
...
/*
philo sleeping and thinking
*/

void sleep_and_think(t_philo *ph)
{
    prthread_mutex_lock(&ph->pa->write_mutex);//look the mutex for writing
    print_status(BLUE"is sleaping\n"CLEAR, ph);

    prthread_mutex_unlock(&ph->pa->write_mutex);
    ft_usleep(ph->pa->time_to_sleep);

    prthread_mutex_lock(&ph->pa->write_mutex);
    print_status(YELLOW"is thinking\n"CLEAR, ph);

    prthread_mutex_unlock(&ph->pa->write_mutex);
}

/*take left fork -> take right fork -> eat -> leave fork -> sleep -< think*/
